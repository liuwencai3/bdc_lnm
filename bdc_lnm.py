...
#应用标题
st.title('A machine learning-based predictive model for predicting lymph node metastasis in patients with bile duct cancer')

# conf
st.sidebar.markdown('## Variables')
WBC = st.sidebar.slider("WBC", 0.00, 20.00, value=5.00, step=0.01)
...
boundary = st.sidebar.selectbox("Boundary",('No','Yes'),index=0)
smoking = st.sidebar.selectbox("Smoking",('No','Yes'),index=0)
ALD = st.sidebar.selectbox("ALD",('No','Yes'),index=0)
...
thyroid_train['lymphonode'] = thyroid_train['lymphonode'].apply(lambda x : +1 if x==1 else 0)
...
features = ['ALD','smoking','boundary','Diameter.G','WBC']#
target = 'lymphonode'
#处理数据不平衡
...
sp = 0.5
#figure
is_t = (XGB.predict_proba(np.array([[ALD,smoking,boundary,Diameter_G,WBC]]))[0][1])> sp
prob = (XGB.predict_proba(np.array([[ALD,smoking,boundary,Diameter_G,WBC]]))[0][1])*1000//1/10
